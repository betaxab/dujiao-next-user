      ...
      <div class="p-6 space-y-5">
        <form @submit.prevent="submitBinding" class="space-y-4">
          ...
          <div class="pt-2">
            ...
            <button type="button" @click="handleLogout" :disabled="isSending || isSubmitting"
              class="mt-3 w-full py-2.5 px-4 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200">
              {{ t('navbar.logout') }}
            </button>
          </div>
        </form>
...
const validateSession = async () => {
  ...
  if (sessionChecking.value) return
  ...
  try {
    ...
    if (profile) {
      userAuthStore.syncUserProfile(profile)
      userProfileStore.profile = profile
    }
    sessionValid.value = true
  } catch {
    // 401 分支会在 API 客户端里清掉 localStorage token，这里只做 store 同步与兜底跳转
    const tokenInStorage = localStorage.getItem('user_token')
    if (!tokenInStorage) {
      sessionValid.value = false
      if (userAuthStore.isAuthenticated) {
        userAuthStore.logout('/auth/login')
      }
    } else {
      // 非鉴权错误（如网络抖动）不强制登出
      sessionValid.value = true
...
const isOpen = computed(() => {
  if (!userAuthStore.isAuthenticated) return false
  if (!sessionReady.value || !sessionValid.value) return false
  const user = userAuthStore.user
  const isBindOnly = user?.email_change_mode === 'bind_only' || (user?.email && user.email.endsWith('@telegram.local'))
